--[[
    ...
    ----------------------------------------------------
    Собственная UI-библиотека для меню в Roblox.
    Никаких внешних загрузок (loadstring/HttpGet) — весь код ваш.

    КУДА ПОЛОЖИТЬ:
    ReplicatedStorage -> вставить как ModuleScript, назвать "UILibrary"

    КАК ИСПОЛЬЗОВАТЬ (из LocalScript):
        local UILibrary = require(game.ReplicatedStorage.UILibrary)
        local Window = UILibrary.CreateLib("Название окна", "Theme1")
        local Tab = Window:NewTab("Вкладка")
        local Section = Tab:NewSection("Секция")
        Section:NewButton("Кнопка", "Описание", function() print("click") end)

    ДОСТУПНЫЕ ЭЛЕМЕНТЫ:
        Section:NewLabel(text)
        Section:NewButton(text, info, callback)
        Section:NewToggle(text, info, callback)              -- callback(state: bool)
        Section:NewSlider(text, info, max, min, callback)     -- callback(value: number)
        Section:NewTextBox(text, info, callback)               -- callback(text: string)
        Section:NewKeybind(text, info, defaultKey, callback)   -- callback()
        Section:NewDropdown(text, info, optionsList, callback) -- callback(option: string)
]]
